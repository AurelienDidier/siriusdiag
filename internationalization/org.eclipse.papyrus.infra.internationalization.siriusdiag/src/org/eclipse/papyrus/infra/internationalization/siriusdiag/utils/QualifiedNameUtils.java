...
package org.eclipse.papyrus.infra.internationalization.siriusdiag.utils;
...
public class QualifiedNameUtils {
	 ...
	 */
	public static EObject getOwner(final DSemanticDiagram diagram) {
	...
	// */
	// private static final Style getPapyrusViewStyle(final DSemanticDiagram diagram) {
	 ...
	 */
	public static DSemanticDiagram getSiriusDiagram(final Resource resource, final String diagramName, final String qualifiedName) {
		if (null != diagramName && !diagramName.isEmpty()) {
			for (final EObject element : resource.getContents()) {
				if (element instanceof DSemanticDiagram) {
					final DSemanticDiagram diagram = (DSemanticDiagram) element;

					if (diagramName.equals(diagram.getName())) {
						if (null == qualifiedName || qualifiedName.isEmpty()
								|| getQualifiedName(diagram.getTarget()).equals(qualifiedName)) {
							// Found
							return diagram;
						}
					}
