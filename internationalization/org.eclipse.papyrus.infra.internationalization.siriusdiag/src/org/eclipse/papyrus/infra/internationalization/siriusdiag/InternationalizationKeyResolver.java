...
package org.eclipse.papyrus.infra.internationalization.siriusdiag;
...
import org.eclipse.papyrus.infra.internationalization.siriusdiag.utils.QualifiedNameUtils;
...
public class InternationalizationKeyResolver extends org.eclipse.papyrus.infra.internationalization.utils.InternationalizationKeyResolver {
	 ...
	 */
	protected static final String LABEL_SIRIUS_DIAGRAM_PREFIX_QN = "_labelSiriusDiagram_"; //$NON-NLS-1$
	 ...
	 */
	protected static final String AIRD_FILE_EXTENSION = "aird"; //$NON-NLS-1$
	...
	public InternationalizationEntry createInternationalizationEntryByKey(final String key, final ResourceSet resourceSet,
		...
		if (key.startsWith(LABEL_SIRIUS_DIAGRAM_PREFIX_QN)) {
			final String keyWithoutPrefix = key.substring(LABEL_SIRIUS_DIAGRAM_PREFIX_QN.length());
			...
			final Resource notationResource = resourceSet
					.getResource(uri.trimFileExtension().appendFileExtension(AIRD_FILE_EXTENSION), true);

			if (null != notationResource && null != notationResource.getContents()
					&& !notationResource.getContents().isEmpty()) {
				final DSemanticDiagram foundDiagram = QualifiedNameUtils.getSiriusDiagram(notationResource, diagramName,
						qualifiedName);
	...
	public String getKey(final InternationalizationEntry entry) {
		...
		if (entry.getKey() instanceof Diagram) {
			result.append(LABEL_SIRIUS_DIAGRAM_PREFIX_QN);
			final DSemanticDiagram diagram = (DSemanticDiagram) entry.getKey();
			final EObject diagramContainer = diagram.getTarget();
