...
public class ControlPropertiesCommand extends AbstractControlCommand {
	...
	protected CommandResult doExecuteWithResult(final IProgressMonitor monitor, final IAdaptable info)
		...
		final Set<Resource> createdResources = new HashSet<>();
	...
	protected List<InternationalizationEntry> getInternationalizationEntries(final Locale locale)
			throws ExecutionException {
		final List<InternationalizationEntry> entries = new ArrayList<>();
		...
		if (null != internationalizationModelResource) {
			...
			for (final DSemanticDiagram table : getSiriusDiagram(oldNotationResource, getRequest().getTargetObject())) {
	 ...
	 */
	protected List<DSemanticDiagram> getSiriusDiagram(final Resource notationResource, final EObject eObject) {
		final List<DSemanticDiagram> siriusDiagrams = new LinkedList<>();
		if (null != notationResource) {
			for (final EObject obj : notationResource.getContents()) {
				if (obj instanceof DSemanticDiagram) {
					DSemanticDiagram siriusDiag = (DSemanticDiagram) obj;
					if (EcoreUtil.isAncestor(eObject, siriusDiag.getTarget())) {
						siriusDiagrams.add(siriusDiag);
					}
				}
			}
		}
